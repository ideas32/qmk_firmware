...
// wpm variables because I don't like the normal implementation of when wpm updates and when it becomes zero

uint8_t timed_wpm = 0;
uint32_t timed_wpm_timer = 0;
...
bool process_record_kb(uint16_t keycode, keyrecord_t *record) {
    ...
    if (record->event.pressed){
        timed_wpm = get_current_wpm();
        timed_wpm_timer = timer_read32();
    }
    return true;
}

// oled is 21 chars wide 4 lines tall
// wpm render function

void render_wpm(void) {

    uint32_t time_diff = abs(timed_wpm_timer - timer_read32());
    if (time_diff >= 1000) {
        set_current_wpm(0);
        timed_wpm = 0;
        timed_wpm_timer = 0;
    ...
    char wpm_str[8];
    snprintf(wpm_str, sizeof(wpm_str), "%d", (int)timed_wpm);
...
// horrortroll code starts here
// oled bongocat animation code shamelessly copied from HorrorTroll's bongocat animation at keyboards/doio/kb16/lib/bongocat.c
...
bool wpm_keycode_kb(uint16_t keycode) {
    if (!wpm_keycode_user(keycode)){
        return wpm_keycode_user(keycode);
    }
    return true; // all keycodes count toward wpm now
}
